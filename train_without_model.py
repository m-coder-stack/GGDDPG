...
import train_params_without_model as params
...
def eval_policy(policy, env_name, seed, eval_episodes=10, writer: SummaryWriter = None):
	eval_env = get_env()
...
if __name__ == "__main__":
	...
	file_name = f"{args.policy}_{args.env}_without_model_{args.seed}_{current_time}"
	logger.info("---------------------------------------")
	...
	evaluations = [eval_policy(policy, args.env, args.seed, writer=writer)]
	...
	for t in range(int(args.max_timesteps)):
		...
		if t < args.start_timesteps:
			...
			if hasattr(policy, "select_position"):
		...
		if (t + 1) % args.eval_freq == 0:
			evaluations.append(eval_policy(policy, args.env, args.seed, writer=writer))
